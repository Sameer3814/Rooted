...
- `data/starter-pack.json` — the curated seed content the app loads on
  first run: **1,040 verses** — the entire Old Testament plus the New
  ...
  (topics linked to related topics), plus **142 characters** with real
...
- `data/characters.json` — the same 142 characters, standalone. Generated
...
- `data/verses.json` — the **full** parsed corpus: the entire Old
  ...
  the user's overlay. Only the 1,040 seed verses are topic-tagged; the
  rest of the corpus isn't yet.
- `data/stories.json` — 14 eras, **202 stories and 465 life events**.
...
9. **Expanding into the New Testament — started 2026-09-10, owner's
   ...
   **Progress: the birth of Jesus is done**, and Jesus's public
   ...
   on the Mount, three major parables, the calming of the storm,
   feeding the five thousand, walking on water, Peter's confession, the
   Transfiguration, and now John's unique material — Nicodemus, the
   woman at the well, and Lazarus raised from the dead. Full detail in
   DATA_MODEL.md §8, items 41 onward, as each installment lands (that's
   now the
