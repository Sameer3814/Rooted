...
- `data/starter-pack.json` — the curated seed content the app loads on
  first run: **1,120 verses** — the entire Old Testament plus the New
  ...
  (topics linked to related topics), plus **155 characters** with real
...
- `data/characters.json` — the same 155 characters, standalone. Generated
...
- `data/verses.json` — the **full** parsed corpus: the entire Old
  ...
  the user's overlay. Only the 1,120 seed verses are topic-tagged; the
  rest of the corpus isn't yet.
- `data/stories.json` — 16 eras, **226 stories and 512 life events**.
...
9. **Expanding into the New Testament — started 2026-09-10, owner's
   ...
   **Progress: all four Gospels are fully curated** — the birth of
   ...
   Commission, and the ascension. A new era, `era_early_church`, has
   opened with Matthias chosen to replace Judas, Pentecost, and Peter
   healing a lame man at the temple gate — Acts is underway. Full
   detail in DATA_MODEL.md §8, items 41 onward, as each
