...
2. ~~Real character portrait illustrations (currently icon placeholders).~~
   **Started 2026-09-15, expanded five times the same day** — 125 of 267
   ...
   arc is positive, negative, or neutral, after Paul's original portrait
   read unhappy despite his story ending in triumph), wired into both
   ...
   shortcut for a partial batch (see DATA_MODEL.md §8, items 91-95).
   Remaining gap: the other ~142 characters (mostly the "deep study"
   supporting cast from item 10) still show the icon placeholder, and
   none of the earlier 49 portraits from before the expression rule
   landed were retroactively redone (an explicit, deliberate scope
   choice, not an oversight — see item 93). Continuing the batch in a
   long-haul multi-session push, same discipline as item 10's
   supporting-cast curation — worth revisiting whether the manifest
   shortcut should graduate to the real `Media` entity once coverage is
   complete, not before.
