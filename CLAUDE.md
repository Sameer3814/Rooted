...
- `data/starter-pack.json` — the curated seed content the app loads on
  first run: **1,078 verses** — the entire Old Testament plus the New
  ...
  (topics linked to related topics), plus **148 characters** with real
...
- `data/characters.json` — the same 148 characters, standalone. Generated
...
- `data/verses.json` — the **full** parsed corpus: the entire Old
  ...
  the user's overlay. Only the 1,078 seed verses are topic-tagged; the
  rest of the corpus isn't yet.
- `data/stories.json` — 15 eras, **214 stories and 487 life events**.
...
9. **Expanding into the New Testament — started 2026-09-10, owner's
   ...
   **Progress: the birth of Jesus is done, and Jesus's ministry is
   complete** — `era_jesus_ministry` runs from his baptism through
   Zacchaeus and the rich young ruler on the road to Jerusalem. A
   second NT era, `era_passion_and_resurrection`, now has the triumphal
   entry, the Last Supper, foot washing, the farewell discourse,
   Gethsemane, the betrayal and arrest, the trial before the council,
   Peter's denial, Judas's remorse, and the trial before Pilate. The
   crucifixion, resurrection, and Acts are next. Full detail in
   DATA_MODEL.md §8, items 41 onward, as
   each installment lands (that's now the authoritative running log —
