...
```sh
py pipeline/parse_books.py                      # genesis + psalms + exodus + ruth + leviticus + numbers + deuteronomy + joshua + judges + 1samuel (what's shipped)
...
- **Books are being added in canonical order** (Genesis, Exodus, Leviticus,
  ...
  it's a genuinely new period, not a continuation of era_judges. Expect the
  lighter pass to come back for the more legal/genealogical stretches later
  on (parts of 1-2 Chronicles, etc.).
